...
loader.load(
    ...
    jugador.scale.set(1, 1, 1); // Ajusta estos números si se ve muy grande/chico
    
    scene.add(jugador);
}, 
// Función opcional para ver el progreso de carga
(xhr) => { console.log((xhr.loaded / xhr.total * 100) + '% cargado'); },
// Función en caso de error
(error) => { console.error('Hubo un error al cargar el modelo:', error); }
);
